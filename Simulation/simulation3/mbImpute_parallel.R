...
# parallel
param_set <- matrix(NA, ncol = 2, nrow = 12)
param_set[,1] = rep(c(0.1, 1, 2), 4)
param_set[,2] = c(rep("Covariate", 3) , rep("Taxa", 3), rep("sample", 3), rep("comb", "3"))
...
parLapply(cl, 1:12,
          function(x){
            ...
            #########generation step##################
            covariate_zi <- read.csv("covariate_zi.csv", row.names = "X")
            taxa_zi <- read.csv("taxa_zi.csv", row.names = "X")
            sample_zi <- read.csv("sample_zi.csv", row.names = "X")
            comb_zi <- read.csv("comb_zi.csv", row.names = "X")
            D <- read.csv("D.csv", row.names = "X")
            meta_data <- read.csv("meta_data.csv", row.names = "X")
            #########finding psi step###################
            if(x <= 3){
              print(x)
              data_fit2(y_sim = covariate_zi, x = meta_data, D = D, psi = as.numeric(param_set[x,1]), k = 5, cond = param_set[x,2])
              rm()
              gc()
            }else if(x <= 6){
              print(x)
              data_fit2(y_sim = taxa_zi, x = meta_data, D = D, psi = as.numeric(param_set[x,1]), k = 5, cond = param_set[x,2])
              rm()
              gc()
            }else if(x <= 9){
              print(x)
              data_fit2(y_sim = sample_zi, x = meta_data, D = D, psi = as.numeric(param_set[x,1]), k = 5, cond = param_set[x,2])
              rm()
              gc()
            }else{
              print(x)
              data_fit2(y_sim = comb_zi, x = meta_data, D = D, psi = as.numeric(param_set[x,1]), k = 5, cond = param_set[x,2])
              rm()
              gc()
            }
          })
